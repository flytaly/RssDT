 ...
 */
const addTitlesToEnclosures = (enclosures) =>
 ...
 * @param {String} info.title - feed title
 * @param {String} [info.theme='default'] - email's theme
...
const composeHTML = (info, feedItems, userFeed = {}) => {
    ...
    const withToC =
        withContentTable === 'DEFAULT' ? withContentTableDefault : withContentTable === 'ENABLE';
    const withItemBody = itemBody === 'DEFAULT' ? itemBodyDefault : itemBody === 'ENABLE';
    const withAttachments =
        attachments === 'DEFAULT' ? attachmentsDefault : attachments === 'ENABLE';
    ...
    const items = feedItems.reduce((acc, _item) => {
        ...
        if (!item.imageUrl) {
            item.imageUrl = getImageFromEnclosures(item.enclosures);
        }

        if (!withAttachments) item.enclosures = [];
        if (item.enclosures) {
            item.enclosures = addTitlesToEnclosures(item.enclosures);
        }
